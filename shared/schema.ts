...
export const passes = pgTable("passes", {
  ...
  passType: text("pass_type").notNull(), // 'daily', 'vehicle', 'crane', 'trailer20', 'trailer40'
...
export const passSchema = z.object({
  ...
  passType: z.enum(["daily", "vehicle", "crane", "trailer20", "trailer40"], {
  ...
  if ((data.passType === "vehicle" || data.passType === "crane" || data.passType === "trailer20" || data.passType === "trailer40") && !data.plateNumber) {
    return false;
  }
  return true;
}, {
  message: "ID number is required for Daily Pass, Vehicle plate number is required for Vehicle/Crane/Trailer passes",
});
