...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Update staff member
  app.put("/api/admin/staff/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // Delete staff member
  app.delete("/api/admin/staff/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/passes", requireAuth, upload.single("slip"), async (req, res) => {
    try {
      ...
      for (const passData of validatedData.passes) {
        ...
        const qrData = `PASS:${passNumber}|CUSTOMER:${passData.customerName}|TYPE:${passData.passType}|VALID:${passData.validDate}|AMOUNT:${PASS_PRICES[passData.passType]}|STAFF:${staffDesignation}|DATE:${new Date().toLocaleDateString()}|STATUS:ACTIVE`;
  ...
  // Reports API
  app.get("/api/reports/daily/:date", requireAuth, async (req, res) => {
    try {
      const { date } = req.params;
      const selectedDate = new Date(date);
      
      const report = await storage.getDailyReport(selectedDate);
      res.json(report);
    } catch (error: any) {
